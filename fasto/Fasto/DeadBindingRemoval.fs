...
let rec removeDeadBindingsInExp (e: TypedExp) : (bool * DBRtab * TypedExp) =
    ...
    | Let(Dec(name, e, decpos), body, pos) ->
        ...
        let nameinbody =
            match SymTab.lookup name buses with
            | Some _ -> true
            | _ -> false

        if (eios || nameinbody) then
            let buses' = SymTab.remove name buses
            (eios || bios, SymTab.combine euses buses', Let(Dec(name, e', decpos), body', pos))
        else
            (bios, buses, body')
